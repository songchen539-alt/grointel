...
import { Loader2, ArrowLeft, Search, BrainCircuit, TrendingUp, Shield, Target, BarChart3, UserCheck } from "lucide-react";
...
export default function CapabilityIntelligenceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (!id) return;
    fetch("/api/capability-intelligence/" + id)
      .then(r => r.json())
  ...
  if (!profile) return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/capability-intelligence" className="text-sm text-blue-400 hover:text-blue-300 mt-4 inline-block">Try another profile</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="mx-auto max-w-4xl px-6 py-8">
        <Link href="/capability-intelligence" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-300 mb-6">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/5 bg-white/[0.02] px-3 py-1 text-xs text-gray-500 mb-3">
            <UserCheck className="h-3 w-3 text-purple-400" /> Capability Intelligence
          </div>
          <h1 className="text-3xl font-bold mb-2">{profile.capability_identity?.name || profile.profile_url}</h1>
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <span className="text-gray-500">{profile.profile_url}</span>
            <span className="text-gray-600">|</span>
            <span className="text-gray-500">{profile.capability_identity?.type || "?"}</span>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {scanProfile ? (
              <div className="space-y-4">
                <Section title="Identity" icon={<Shield className="h-3.5 w-3.5 text-purple-400" />}>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div><span className="text-gray-500">Name:</span> {scanProfile.display_name || "?"}</div>
                    <div><span className="text-gray-500">URL:</span> {scanProfile.profile_url}</div>
                    <div><span className="text-gray-500">Type:</span> {scanProfile.entity_type || "?"}</div>
                    <div><span className="text-gray-500">Domain:</span> {scanProfile.normalized_domain || "?"}</div>
                  </div>
                </Section>
                {scanProfile.public_summary && <Section title="Summary"><p className="text-sm">{scanProfile.public_summary}</p></Section>}
                {scanProfile.detected_capabilities?.length > 0 && <Section title="Detected Capabilities"><JsonInline data={scanProfile.detected_capabilities} /></Section>}
                {scanProfile.detected_audiences?.length > 0 && <Section title="Detected Audiences"><JsonInline data={scanProfile.detected_audiences} /></Section>}
                ...
                {scanProfile.detected_channels?.length > 0 && <Section title="Detected Channels"><JsonInline data={scanProfile.detected_channels} /></Section>}
                {scanProfile.public_evidence?.length > 0 && <Section title="Public Evidence"><JsonInline data={scanProfile.public_evidence} /></Section>}
                ...
                <Section title="Scan Confidence">
                  {Object.keys(scanConf).length > 0 ? (
                    <div className="space-y-1">
                      {Object.entries(scanConf).map(([key, val]) => (<ConfidenceBadge key={key} name={key.charAt(0).toUpperCase() + key.slice(1)} score={val as number} />))}
                    </div>
          ...
          {/* Section 2: Capability Knowledge */}
          <div className="border-t border-white/5 pt-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <BrainCircuit className="h-4 w-4 text-gray-500" /> Capability Knowledge
            </h2>
            <div className="space-y-4">
              <Section title="Identity" icon={<Shield className="h-3.5 w-3.5 text-purple-400" />}><JsonInline data={profile.capability_identity} /></Section>
              <Section title="Capability DNA" icon={<TrendingUp className="h-3.5 w-3.5 text-green-400" />}><JsonInline data={profile.capability_dna} /></Section>
              <Section title="Audience DNA" icon={<Target className="h-3.5 w-3.5 text-yellow-400" />}><JsonInline data={profile.audience_dna} /></Section>
              <Section title="Evidence Summary"><JsonInline data={profile.evidence_summary} /></Section>
              <Section title="Strengths"><JsonInline data={profile.strengths} /></Section>
              {profile.limitations?.length > 0 && <Section title="Limitations"><JsonInline data={profile.limitations} /></Section>}
              {profile.preferred_collaborations?.length > 0 && <Section title="Preferred Collaborations"><JsonInline data={profile.preferred_collaborations} /></Section>}
              <Section title="Pricing Signals"><JsonInline data={profile.pricing_signals} /></Section>
              <Section title="Availability Signals"><JsonInline data={profile.availability_signals} /></Section>
              <Section title="Knowledge Confidence">
                {Object.keys(knowledgeConf).length > 0 ? (
                  <div className="space-y-1">
                    {Object.entries(knowledgeConf).map(([key, val]) => (<ConfidenceBadge key={key} name={key.replace(/_/g, " ").replace(/\b\w/g, (c: any) => c.toUpperCase())} score={val as number} />))}
                  </div>
