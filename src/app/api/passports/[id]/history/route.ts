...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const r = await fetch(u + "/rest/v1/growth_capability_history?select=*&passport_id=eq." + encodeURIComponent(id) + "&order=calculated_at.desc", { headers: h(), cache: "no-store" });
    if (!r.ok) return NextResponse.json({ success: false, error: "Query failed: " + r.status }, { status: 500 });
    return NextResponse.json({ success: true, history: await r.json() });
