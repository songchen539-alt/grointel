// GroIntel Admin Growth Needs Detail API
...
const allowedFields = new Set(["status", "notes", "growth_goal", "target_market", "target_customer", "current_challenge", "budget_min", "budget_max", "timeline"]);

export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  if (!serviceKey || !supabaseUrl) return NextResponse.json({ success: false, error: "Not configured" }, { status: 500 });
  try {
    const res = await fetch(supabaseUrl + "/rest/v1/company_growth_needs?select=*&id=eq." + encodeURIComponent(id), { headers: sbH(), cache: "no-store" });
    ...
    return NextResponse.json({ success: true, need: rows[0] });
  } catch {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(supabaseUrl + "/rest/v1/company_growth_needs?id=eq." + encodeURIComponent(id), {
      method: "PATCH",
    ...
    const rows = await res.json();
    return NextResponse.json({ success: true, need: rows[0] || rows });
  } catch {
    return NextResponse.json({ success: false, error: "Server error" }, { status: 500 });
  }
}
