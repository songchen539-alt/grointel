// GroIntel Admin Match Detail API
...
const allowedFields = new Set([
  "status", "match_score", "admin_notes", "match_reason",
  "recommended_solution_type", "service_id", "updated_at",
]);
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(supabaseUrl + "/rest/v1/growth_matches?select=*&id=eq." + encodeURIComponent(id), { headers: sbH(), cache: "no-store" });
    ...
    return NextResponse.json({ success: true, match: rows[0] });
  } catch {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(supabaseUrl + "/rest/v1/growth_matches?id=eq." + encodeURIComponent(id), {
      method: "PATCH",
    ...
    const rows = await res.json();
    return NextResponse.json({ success: true, match: rows[0] || rows });
  } catch {
    return NextResponse.json({ success: false, error: "Server error" }, { status: 500 });
  }
}
