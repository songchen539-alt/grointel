// GET /api/business-intelligence — list latest knowledge profiles
import { NextResponse } from "next/server";
import { DbBusinessKnowledgeProfile } from "@/lib/db/types";
...
export async function GET() {
  ...
  try {
    const r = await fetch(u + "/rest/v1/business_knowledge_profiles?select=*&order=created_at.desc&limit=50", { headers: h(), cache: "no-store" });
    if (!r.ok) return NextResponse.json({ success: false, error: "Table may not exist" }, { status: 500 });
    const rows: DbBusinessKnowledgeProfile[] = await r.json();
