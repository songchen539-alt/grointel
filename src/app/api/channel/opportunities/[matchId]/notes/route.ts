// POST /api/channel/opportunities/[matchId]/notes
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  await fetch(supabaseUrl + "/rest/v1/channel_opportunity_events", {
    method: "POST", headers: { ...sbH(), "Prefer": "return=minimal" },
    body: JSON.stringify([{ match_id: matchId, channel_id: body.channelId, event_type: "channel_left_note", note: body.note, metadata: {}, created_at: now }]),
  });
