// POST /api/channel/opportunities/[matchId]/decline
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  await Promise.all([
    fetch(supabaseUrl + "/rest/v1/growth_matches?id=eq." + encodeURIComponent(matchId), {
      ...
      body: JSON.stringify({ status: "channel_declined", updated_at: now }),
    }),
    fetch(supabaseUrl + "/rest/v1/channel_opportunity_events", {
      method: "POST", headers: { ...sbH(), "Prefer": "return=minimal" },
      body: JSON.stringify([{ match_id: matchId, channel_id: body.channelId, event_type: "channel_declined_opportunity", note: body.note || "", metadata: {}, created_at: now }]),
    }),
  ]);
