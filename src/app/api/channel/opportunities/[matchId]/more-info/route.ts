// POST /api/channel/opportunities/[matchId]/more-info
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ matchId: string }> }) {
  ...
  if (!body.note) return NextResponse.json({ success: false, error: "Note required" }, { status: 400 });
  ...
  await Promise.all([
    fetch(supabaseUrl + "/rest/v1/growth_matches?id=eq." + encodeURIComponent(matchId), {
      ...
      body: JSON.stringify({ status: "channel_requested_more_info", updated_at: now }),
    }),
    fetch(supabaseUrl + "/rest/v1/channel_opportunity_events", {
      method: "POST", headers: { ...sbH(), "Prefer": "return=minimal" },
      body: JSON.stringify([{ match_id: matchId, channel_id: body.channelId, event_type: "channel_requested_more_info", note: body.note, metadata: {}, created_at: now }]),
    }),
  ]);
