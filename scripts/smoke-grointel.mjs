...
async function main() {
  ...
  assert((dailyIngestion.liveDiscovery?.sources || []).some((source) => source.source === "web3_event_pages"), "daily ingestion should include Web3 event pages as live community and event supply sources");
  if (dailyIngestion.liveDiscovery?.success) {
    ...
    assert(dailyIngestion.liveDiscovery.qualitySummary?.highQualityCount > 0, "successful live discovery should quality-score live candidates");
    assert(dailyIngestion.liveDiscovery.qualitySummary?.coveredSources > 0, "successful live discovery should expose live source coverage");
  }
